...
def drive_zijian(c):
    ...
    pLateralOffset = 0.5
    pAngleOffset = 3
    R['steer'] = -pLateralOffset *(S['trackPos'] + lateralSetPoint) + pAngleOffset * S['angle']
    R['steer'] = np.tanh(R['steer'])


    MAX_SPEED = 200
    MIN_SPEED = 10
    target_speed = 150 #MIN_SPEED + speed_target * (MAX_SPEED - MIN_SPEED)

    if S['speedX'] > target_speed:
        R['brake'] = - 0.1 * (target_speed - S['speedX'])
        R['brake'] = np.tanh(R['brake'])
        R['accel'] = 0
    else:
        R['brake'] = 0
        R['accel'] = 0.05 * (target_speed - S['speedX'])
        if S['speedX'] < target_speed - (R['steer']*50):
            R['accel']+= .01
        if S['speedX'] < 10:
           R['accel']+= 1/(S['speedX'] +.1)
        R['accel'] = np.tanh(R['accel'])
